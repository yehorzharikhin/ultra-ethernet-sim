...
The simulation reveals that while standard **ECMP suffers catastrophic packet loss (>90%)** under synchronized "permutation" traffic loads, **Adaptive Routing** combined with **Congestion Control** can maintain network stability where other algorithms fail.
...
*Results from "Kill Test" Scenario (200% Oversubscription, Flow Window=100)*

| Routing Algorithm | Congestion Control | Drop Rate | Avg Latency | P99 Latency | Outcome |
| :--- | :---: | :---: | :---: | :---: | :--- |
| **ECMP (Baseline)** | ❌ | **98.08%** | 260 µs | 1073 µs | **Collapsed** (*Latency biased by survival*) |
| **Packet Spraying** | ❌ | 87.43% | 46 ms | 315 ms | **Failed** (HOL Blocking & Buffer Saturation) |
| **Pure Adaptive** | ❌ | 83.08% | 9 ms | 78 ms | **Struggling** (High latency due to full buffers) |
| **Adaptive + CC** | ✅ | **11.63%** | **289 µs** | **3065 µs** | **STABLE** (Lowest effective latency) |

### 📉 Reproduction
To replicate the experiments and generate the visualizations:

**Option 1: Jupyter/Colab (Recommended)**
Open `Ultra_Ethernet_Congestion_Study.ipynb`, install dependencies, and run all cells. The notebook will generate the simulation data and render the visualization charts inline.
...
### 📈 Visualization
![Results](results/main_results_grid.png)
